...
`--mode archival --confirm-private-archival` crosses a distinct private retention boundary. It takes a consistent SQLite backup and retains raw media, audio, frames, captions, canonical records, and selected non-secret intermediates while excluding cookies, credentials, authentication artifacts, caches, locks, temporary files, task lease directories, rendered behavior targets, and generated Skill files. A selected edition contributes only its sanitized edition identity and its own records/builds alongside shared Analyze evidence. The export never serializes edition output, project-root, or install-root paths.
