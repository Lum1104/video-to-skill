...
Evidence-bundle export is a separate deterministic packaging surface. `evidence-bundle WORKSPACE --mode compact --output FILE.v2sbundle` synthesizes a shareable allowlist from SQLite and canonical records: sanitized source descriptors, semantic and curriculum state, observations and gaps, selected visual derivatives and contact sheets, review/build reports, and the same sanitized tool-run projection. Normalized transcripts and caption files are absent unless `--authorize-transcript-redistribution` is explicit. Compact bundles never traverse raw workspace trees and never contain media, audio, SQLite, task packets, behavior targets, credentials, host paths, or generated Skill files.

`--mode archival --confirm-private-archival` crosses a distinct private retention boundary. It constructs a consistent evidence-only SQLite database and retains raw media, audio, frames, normalized transcript evidence, explicitly declared external local caption sidecars, canonical records, and selected non-secret intermediates while excluding cookies, credentials, authentication artifacts, caches, locks, temporary files, task lease directories, rendered behavior targets, and generated Skill files. An edition-scoped export records the selected edition identity and adds its records and build reports; because archival export preserves workspace-wide canonical history, shared legacy canonical records and build reports can also be present. The export never serializes edition output, project-root, or install-root paths.

Both modes use an uncompressed deterministic ZIP container with the `.v2sbundle` suffix. Fixed member metadata, sorted safe paths, SHA-256 and size records, source/depth/edition lineage, and a content-derived bundle ID make repeated exports byte-identical while workspace state is unchanged. Every input is opened beneath a trusted root without following symlinks; special files, duplicate members, absolute paths, and traversal components are rejected. Publication uses a mode-`0600` temporary file and an atomic create-only hard link outside the workspace; a concurrent or repeated export succeeds only when the existing target is byte-for-byte identical. `verify-evidence-bundle` validates the safe archive shape, manifest identity, member set, sizes, and checksums without extracting. Bundle mode and the analysis-depth profile are independent contracts.
...
Media and frames are private, reproducible cache artifacts. `clean` removes those files but not the database, captions, manifests, or generated skill. Create a confirmed private archival bundle before cleaning only when long-term preservation was explicitly requested.
...
`run` and `submit` are the generation boundary. The coordinator materializes bounded Analyze, curriculum-planning Author, full Author, behavior-trial Review, and independent Review tasks in the workspace; validates each result against its task snapshot, lease, schema, and evidence scope; and advances immutable canonical heads. The former `blueprint-schema` and `build-skill` commands are not public CLI surfaces.

The internal schema-version 2 `CourseSkillBlueprint` is compiled only after the complete checkpointed workflow has passed. Canonical state supplies:
...
- one recommended curriculum plus up to two justified alternate paths over the same semantic map;
- an evidence-driven artifact plan in which every separate file states why independent loading is useful.

The compiler recomputes the source ledger, verifies canonical record and draft digests, reconstructs the current behavior catalog, rehashes every trial and the immutable behavior target, and requires final delivery bytes to match the reviewed target. It rejects omissions, additions, metadata changes, stale workspace identity, unsupported coverage upgrades, invalid semantic links, material semantic units that silently disappear, incomplete curriculum or behavior checkpoints, and failed independent Review. It does not enforce a fixed chapter count or an artifact quota per interaction mode.
