...
Platform cookies and provider keys are opt-in. Prefer environment variables and a dedicated browser profile. Do not pass workspaces or debug logs to third parties before reviewing them.
...
Selected visual assets must resolve to regular JPEG, PNG, or WebP files inside the evidence workspace without crossing a symbolic link. Inputs are capped at 20 MiB, 4096 pixels on either axis, and 16,000,000 pixels total; animation and unsupported decoded formats are rejected. Accepted images are decoded and re-encoded as PNG before entering the shareable package, with a 20 MiB total output bound.
